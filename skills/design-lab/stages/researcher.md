...
**前置**：先 Read Step 2 的设计产物（我们有哪些问题、选了哪些模式、结构设计），带着问题清单去搜——不是"这项目不错"，而是"这项目解决的是不是我们清单里的问题"。**存量模式（Step 1 已判定）内部复用优先**：项目自身代码是第一复用来源（同款问题拟合四问+裁剪清单照用），GitHub 只补内部覆盖不了的缺口。
